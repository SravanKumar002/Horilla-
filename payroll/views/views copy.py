...
def payslip_inline_edit(request, payslip_id):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        # CRITICAL: Recalculate Housing Allowance, Transport Allowance, and Other Allowance based on Paid Days

        # Apply the same formula as basic_pay

        if should_recalculate and days_in_period > 0 and paid_days >= 0:

            if is_imported:

                # For imported payslips, use monthly values from pay_head_data or reverse-calculate from current values
                ...
                if calendar_month_days > 0:
                    ...
                    # Get monthly values or use current values as monthly (if monthly values not stored)

                    monthly_housing = pay_data.get("monthly_housing_allowance", 0)

                    if monthly_housing == 0:

                        # Use current value from pay_head_data or payslip as monthly value

                        monthly_housing = pay_data.get("housing_allowance", payslip.housing_allowance or 0)

                   

                    monthly_transport = pay_data.get("monthly_transport_allowance", 0)

                    if monthly_transport == 0:

                        monthly_transport = pay_data.get("transport_allowance", payslip.transport_allowance or 0)

                   

                    monthly_other = pay_data.get("monthly_other_allowance", 0)

                    if monthly_other == 0:

                        monthly_other = pay_data.get("other_allowance", payslip.other_allowance or 0)

                   

                    # Recalculate based on paid_days

                    if monthly_housing > 0:

                        per_day_housing = monthly_housing / calendar_month_days

                        recalculated_housing = round(per_day_housing * paid_days, 2)

                        data["housing_allowance"] = recalculated_housing

                   

                    if monthly_transport > 0:

                        per_day_transport = monthly_transport / calendar_month_days

                        recalculated_transport = round(per_day_transport * paid_days, 2)

                        data["transport_allowance"] = recalculated_transport

                   

                    if monthly_other > 0:

                        per_day_other = monthly_other / calendar_month_days

                        recalculated_other = round(per_day_other * paid_days, 2)

                        data["other_allowance"] = recalculated_other

            else:

                # For non-imported payslips, use payslip model fields as monthly values

                # Formula: Allowance = (Monthly Allowance / Days in Period) × Paid Days

                monthly_housing = payslip.housing_allowance or pay_data.get("housing_allowance", 0)

                monthly_transport = payslip.transport_allowance or pay_data.get("transport_allowance", 0)

                monthly_other = payslip.other_allowance or pay_data.get("other_allowance", 0)

               

                if monthly_housing > 0:

                    per_day_housing = monthly_housing / days_in_period
                ...
                if monthly_transport > 0:
                    ...
                    per_day_transport = monthly_transport / days_in_period
                ...
                if monthly_other > 0:
                    ...
                    per_day_other = monthly_other / days_in_period
        ...
        success_message = _("Payslip updated successfully")

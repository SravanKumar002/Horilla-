...
def payslip_import_template(request):
    """
    Download a company-wise payslip import template in Excel format.

    Matches the structure from payslip_import_download with:
    - Static components (green): Employee Id, Employee Name, Department, Basic Salary, 
      Housing Allowance, Transport Allowance, Other Allowance, Gross Salary
    - Variable components (orange): Total Paid Days, Loss of Pay, salary_advance_loan_recovery,
      Deduction, Overtime, salary_advance, bonus, Net Salary
    - Merged headers: DEDUCTION and ADDITION
    - Color coding and explanatory text
    - Formulas for Gross Salary and Net Salary that work after editing

    - Uses the currently selected company from the navbar (session 'selected_company')
      or optional ?company_id query param.
    - Prefills Employee Id, Employee Name, and allowance defaults for all employees.
    """
    company_id = request.GET.get("company_id") or request.session.get("selected_company")

    # Let HorillaCompanyManager scope employees by selected company automatically.
    ...
    # Column order matching the image exactly: Static (green) then Variable (orange)
    # Static: Employee Id, Employee Name, Department, Basic Salary, Housing Allowance, Transport Allowance, Other Allowance, Gross Salary
    # Variable: Total Paid Days, Loss of Pay, salary_advance_loan_recovery, Deduction, Overtime, salary_advance, bonus, Net Salary
    export_rows = []
    ...
    for emp in employees:
        ...
        housing_default = float(emp.get("employee_work_info__housing_allowance") or 0)
        transport_default = float(emp.get("employee_work_info__transport_allowance") or 0)
        other_default = float(emp.get("employee_work_info__other_allowance") or 0)
        
        export_rows.append({
            "Employee Id": emp_id,
            "Employee Name": emp_name,
            "Department": dept_name,
            "Basic Salary": round(basic_pay, 2),
            "Housing Allowance": round(housing_default, 2),
            "Transport Allowance": round(transport_default, 2),
            "Other Allowance": round(other_default, 2),
            "Gross Salary": "",  # Will be filled with formula
            # Variable components (all empty for template) - order matches image
            "Total Paid Days": "",
            "Loss of Pay": "",
            ...
            "salary_advance": "",
            "bonus": "",
            "Net Salary": "",  # Will be filled with formula
        })

    df = pd.DataFrame(export_rows)
    file_name = f"payslip_import_template_{company_id or 'all'}.xlsx"
    ...
    # Formatting with color coding: Static (green) vs Variable (orange)
    workbook = writer.book
    worksheet = writer.sheets["Payslips"]
    ...
    # Static components data format (green) - for data rows
    static_data_fmt = workbook.add_format({
        'bg_color': '#D8E4BC',  # Light green
        'border': 1,
        'align': 'left',
    ...
    # Variable components data format (orange) - for data rows
    variable_data_fmt = workbook.add_format({
        'bg_color': '#FCD5B4',  # Orange
        'border': 1,
        'align': 'left',
    ...
    # Formula format for static components (green background)
    static_formula_fmt = workbook.add_format({
        'bg_color': '#D8E4BC',  # Light green
        'border': 1,
        'align': 'left',
        'valign': 'vcenter'
    ...
    # Formula format for variable components (orange background)
    variable_formula_fmt = workbook.add_format({
        'bg_color': '#FCD5B4',  # Orange
        'border': 1,
        'align': 'left',
        'valign': 'vcenter'
    ...
    # Merged header format for DEDUCTION and ADDITION
    merged_header_fmt = workbook.add_format({
        'bold': True,
        'align': 'center',
        'valign': 'vcenter',
        'border': 1
    ...
    # Static columns (0-7): Employee Id, Employee Name, Department, Basic Salary, Housing Allowance, Transport Allowance, Other Allowance, Gross Salary
    static_cols = ["Employee Id", "Employee Name", "Department", "Basic Salary", "Housing Allowance", "Transport Allowance", "Other Allowance", "Gross Salary"]
    # Variable columns (8-15): Total Paid Days, Loss of Pay, salary_advance_loan_recovery, Deduction, Overtime, salary_advance, bonus, Net Salary
    variable_cols = ["Total Paid Days", "Loss of Pay", "salary_advance_loan_recovery", "Deduction", "Overtime", "salary_advance", "bonus", "Net Salary"]
    
    # Row 0: Merged headers (DEDUCTION and ADDITION)
    # DEDUCTION spans columns 9-11 (Loss of Pay, salary_advance_loan_recovery, Deduction)
    # ADDITION spans columns 12-14 (Overtime, salary_advance, bonus)
    # Note: Net Salary (col 15) is separate, not under ADDITION merged header
    worksheet.merge_range(0, 9, 0, 11, "DEDUCTION", merged_header_fmt)
    worksheet.merge_range(0, 12, 0, 14, "ADDITION", merged_header_fmt)
    
    # Row 2: Write column headers with appropriate formatting (startrow=2 means headers are at row 2)
    for col_num, col_name in enumerate(df.columns.values):
        if col_name in static_cols:
            worksheet.write(2, col_num, col_name, static_header_fmt)
        elif col_name in variable_cols:
            worksheet.write(2, col_num, col_name, variable_header_fmt)
        else:
            # Default format for any unexpected columns
            default_fmt = workbook.add_format({'bold': True, 'bg_color': '#D3D3D3', 'border': 1})
            worksheet.write(2, col_num, col_name, default_fmt)
        worksheet.set_column(col_num, col_num, 18)
    
    # Apply color formatting and formulas to all data rows (rows 3 onwards, since startrow=2 and header is at row 2)
    # Static columns (0-7) get green background, Variable columns (8-15) get orange background
    num_data_rows = len(df)
    for row_idx in range(num_data_rows):
        data_row = row_idx + 3  # Data starts at row 3 (0-indexed), since header is at row 2
        # Convert data_row to 1-indexed Excel row number (needed for formulas)
        ...
        for col_num, col_name in enumerate(df.columns.values):
            ...
            if col_name in static_cols:
                # Static columns: green background
                if col_name == "Gross Salary":
                    # Column H (index 7): Gross Salary formula
                    # Gross Pay = Housing Allowance + Transport Allowance + Other Allowance + Overtime + Salary Advance + Bonus
                    # Excel columns: E=4 (Housing), F=5 (Transport), G=6 (Other), M=12 (Overtime), N=13 (Salary Advance), O=14 (Bonus)
                    gross_formula = f"=SUM(D{excel_row},E{excel_row},F{excel_row},G{excel_row})"
                    worksheet.write_formula(data_row, col_num, gross_formula, static_formula_fmt)
                else:
                    # Handle different value types for other static columns
                    if pd.isna(cell_value) or cell_value == "" or (isinstance(cell_value, str) and cell_value.strip() == ""):
                        worksheet.write_blank(data_row, col_num, None, static_data_fmt)
                    elif isinstance(cell_value, (int, float)):
                        worksheet.write_number(data_row, col_num, float(cell_value), static_data_fmt)
                    else:
                        worksheet.write(data_row, col_num, str(cell_value), static_data_fmt)
            elif col_name in variable_cols:
                # Variable columns: orange background
                if col_name == "Net Salary":
                    # Column P (index 15): Net Salary formula
                    # Net Pay = Gross Pay - (Loss of Pay + Loan Recovery + Deduction)
                    # Excel columns: H=7 (Gross Salary), J=9 (Loss of Pay), K=10 (Loan Recovery), L=11 (Deduction)
                    # excel_row is already defined above
                    net_formula = f"=SUM(M{excel_row},N{excel_row},O{excel_row},H{excel_row})-SUM(J{excel_row}+K{excel_row}+L{excel_row})"
                    worksheet.write_formula(data_row, col_num, net_formula, variable_formula_fmt)
                else:
                    # Handle different value types for other variable columns
                    if pd.isna(cell_value) or cell_value == "" or (isinstance(cell_value, str) and cell_value.strip() == ""):
                        worksheet.write_blank(data_row, col_num, None, variable_data_fmt)
                    elif isinstance(cell_value, (int, float)):
                        worksheet.write_number(data_row, col_num, float(cell_value), variable_data_fmt)
                    else:
                        worksheet.write(data_row, col_num, str(cell_value), variable_data_fmt)
            else:
                # Default format for unexpected columns
                default_data_fmt = workbook.add_format({'bg_color': '#D3D3D3', 'border': 1})
                if pd.isna(cell_value) or cell_value == "" or (isinstance(cell_value, str) and cell_value.strip() == ""):
                    worksheet.write_blank(data_row, col_num, None, default_data_fmt)
                elif isinstance(cell_value, (int, float)):
                    worksheet.write_number(data_row, col_num, float(cell_value), default_data_fmt)
                else:
                    worksheet.write(data_row, col_num, str(cell_value), default_data_fmt)
    
    # Set row heights
    worksheet.set_row(0, 20)  # Merged header row
    worksheet.set_row(1, 5)   # Spacing row
    worksheet.set_row(2, 25)  # Column header row
    
    # Add labels row below data
    last_row = len(df) + 2  # +2 because we start at row 2
    label_fmt_green = workbook.add_format({'bg_color': '#D8E4BC', 'bold': True, 'border': 1})
    label_fmt_orange = workbook.add_format({'bg_color': '#FCD5B4', 'bold': True, 'border': 1})
    
    # Write labels in row after data (matching payslip_import_download structure)
    worksheet.merge_range(last_row + 1, 0, last_row + 1, 7, "Static components for every payroll month", label_fmt_green)
    worksheet.merge_range(last_row + 1, 8, last_row + 1, 15, "Variable components to be changed/entered every month", label_fmt_orange)
        ...
        with transaction.atomic():
            for index, row in df.iterrows():
                ...
                try:
                    ...
                    basic_pay = round(per_day_basic_pay * paid_days, 2)

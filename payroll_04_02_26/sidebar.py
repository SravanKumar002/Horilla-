...
SUBMENUS = [
    ...
    {
        "menu": trans("Run Payroll"),
        "redirect": reverse("payslip-info-import"),
        "accessibility": "payroll.sidebar.runpayroll_accessibility",
    },
    {
        "menu": trans("Payslips"),
...
def runpayroll_accessibility(request, submenu, user_perms, *args, **kwargs):
    return (
        request.user.has_perm("payroll.payslip-info-import")
        or request.user.groups.filter(name="Company Admin").exists()
    )

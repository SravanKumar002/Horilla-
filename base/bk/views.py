...
def calendar_work_records_change_month(request):
    ...
    # --- Batch holiday query: get holidays which overlap with any day in this month
    ...
    # Build a set of all dates covered by all holidays for fast lookup
    holiday_dates_set = set()
    for hol in all_holidays:
        # For each holiday, add every day from start_date to end_date inclusive, limited to this month
        hol_start = max(hol.start_date, month_dates[0])
        hol_end = min(hol.end_date or hol.start_date, month_dates[-1])
        day = hol_start
        while day <= hol_end:
            holiday_dates_set.add(day)
            day += timedelta(days=1)

    # Get employee joining date for filtering
    joining_date = None
    ...
    for week_index, week in enumerate(cal):
        for weekday_index, d in enumerate(week):
            ...
            # MULTI-DAY holiday check: If date_obj is in any holiday date range, mark as public-holiday
            ...
            # Prioritize leave records - if there's an approved leave, show it
            # (even if WorkRecord exists but doesn't have leave type set)
            if leave_info:
                # There's an approved leave - show leave status
                if date_obj > today:
                    # Future leave
                    day_status = "leave-half" if leave_info["is_half_day"] else "leave-full"
                else:
                    # Past/current leave - check attendance for half-day
                ...
                # No approved leave, but WorkRecord exists - use WorkRecord
                if date_obj > today:
                    if wr.work_record_type == "WOF":
                        day_status = "weekly-off"
                    elif wr.work_record_type == "LFD":
                ...
                # No work record and no leave - determine status based on shift schedule
                if emp_shift:
                    day_name = date_obj.strftime("%A").lower()
                    is_working_day = EmployeeShiftSchedule.objects.filter(
                        shift_id=emp_shift, day__day=day_name

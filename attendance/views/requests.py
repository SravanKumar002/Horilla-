...
def build_month_overview(request, attendances_queryset):
    ...
    for day_number in range(1, last_day + 1):
        ...
        # ⭐ PRIORITY 2: Check for approved leave
            ...
            # - request_description is set (contains rejection reason)
            # - is_validate_request is False (no longer pending)
            # - is_validate_request_approved is False (not approved)
            elif att.request_description and not att.is_validate_request and not att.is_validate_request_approved:
                status_key = "rejected"
                types = ["Rejected"]
...
def cancel_attendance_request(request, attendance_id):
    """
    This method is used to cancel/reject attendance request.
    
    GET: Shows the rejection form in a modal
    POST: Processes the rejection with a reason
    """
    try:
        attendance = Attendance.objects.get(id=attendance_id)
    ...
    # Handle POST request - process rejection
    if request.method == "POST":
        form = AttendanceRejectForm(request.POST)
        if form.is_valid():
            rejection_reason = form.cleaned_data.get("reason", "")
            
            # Save request_type before clearing it
            request_type = attendance.request_type
            
            # Store rejection reason in request_description
            attendance.request_description = rejection_reason
            ...
            if request_type == "create_request":
                attendance.rejected_date = timezone.now()
                attendance.save()
            ...
            # Return success response with reload trigger for HTMX
            response = HttpResponse("<script>location.reload();</script>")
            response["HX-Trigger"] = "reload-dashboard-notifications"
            return response
        else:
            # Form is invalid, render form again with errors
            return render(
                request,
                "requests/attendance/cancel_form.html",
                {"form": form, "attendance_id": attendance_id}
            )
    
    # Handle GET request - show rejection form
    form = AttendanceRejectForm()
    return render(
        request,
        "requests/attendance/cancel_form.html",
        {"form": form, "attendance_id": attendance_id}

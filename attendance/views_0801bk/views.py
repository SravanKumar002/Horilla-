...
def attendance_view(request):
    ...
    # Only show attendances where validation request was submitted and approved
    # Must have both: is_validate_request_approved=True AND requested_data IS NOT NULL
    # This ensures a request was actually submitted (requested_data) and approved
    ...
    # Only show overtime attendances where validation request was submitted and approved
    # Must have both: is_validate_request_approved=True AND requested_data IS NOT NULL
    ot_attendances = Attendance.objects.filter(
...
def validate_this_attendance(request, obj_id):
    ...
    try:
        ...
        # If there's a pending validation request, redirect to request-attendance-view
        if attendance.is_validate_request:
            return HttpResponseRedirect(
                reverse("request-attendance-view") + f"?id={attendance.id}"
            )
...
def user_request_one_view(request, id):
    ...
    try:
        ...
        raise Http404(_("Invalid attendance reference."))

    try:
        attendance_request = Attendance.objects.get(id=attendance_id)
    except Attendance.DoesNotExist:
        raise Http404(_("Attendance record not found."))

    # --- Fix for at_work_second possibly being None ---
    # Use 0 if the field is None to prevent TypeError
    ...
    # --- Fix for overtime_second possibly being None ---
    # Use 0 if the field is None to prevent TypeError
    ...
    late_entries = attendance_request.late_come_early_out.all()
    type_labels = [force_str(entry.get_type_display()) for entry in late_entries]
    if not type_labels:
        type_labels = [force_str(_("On Time"))]

    if attendance_request.is_validate_request_approved:
        status_key = "approved"
        status_display = _("Approved")
    elif attendance_request.attendance_validated:
        status_key = "validated"
        status_display = _("Regularized")
    elif attendance_request.is_validate_request:
        status_key = "requested"
        status_display = _("Requested")
    else:
        status_key = "not_regularized"
        status_display = _("Not Regularized")

    pending_hour = attendance_request.hours_pending()
    
    # Ensure json is imported and closest_numbers is defined elsewhere or imported
    instance_ids_json = request.GET.get("instances_ids")
    instance_ids: list[int] = []
    if instance_ids_json:
        parsed_ids = None
        try:
            parsed_ids = json.loads(instance_ids_json)
        except json.JSONDecodeError:
            cleaned = instance_ids_json.strip().strip("[]")
            if cleaned:
                parsed_ids = [value.strip() for value in cleaned.split(",")]
    ...
    if instance_ids:
        safe_instances_json = json.dumps(instance_ids)
    else:
        safe_instances_json = "[]"

    previous_instance, next_instance = closest_numbers(instance_ids, attendance_id)
    ...
    return render(
        ...
        "attendance/attendance/attendance_request_one.html",
        {
            ...
            "at_work": at_work,
            "over_time": over_time,
            "type_display": ", ".join(type_labels),
            "status_display": status_display,
            "status_key": status_key,
            "pending_hour": pending_hour,
            "previous_instance": previous_instance,
            "next_instance": next_instance,
            "instance_ids_json": safe_instances_json,

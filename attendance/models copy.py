...
class Attendance(HorillaModel):
    ...
    def handle_overtime_conditions(self):
        ...
        def save(self, *args, **kwargs):
            """
            Safe save for Attendance model — prevents crashes if employee_id is None.
            """
            # 🧩 Guard: if employee_id is missing, just save and exit
            if not getattr(self, "employee_id", None):
                print("⚠️ Attendance.save() called with no employee_id — skipping overtime check.")
                super().save(*args, **kwargs)
                return

            try:
                # Your normal overtime logic — wrap safely
                employee_ot = getattr(self.employee_id, "employee_overtime", None)
                if employee_ot is not None:
                    employee_ot.filter(...)  # keep your existing logic here
                else:
                    print(f"⚠️ Employee {self.employee_id_id} has no employee_overtime relation.")
            except Exception as e:
                print(f"⚠️ Overtime logic skipped due to error: {e}")
